...
WHERE 
    status = '即將授課'
    AND booking_at = '2024-11-24T17:10:25.000Z'
...
FROM 
    "USER"
INNER JOIN "CREDIT_PURCHASE" ON "CREDIT_PURCHASE".user_id = "USER".id
WHERE user_id = (SELECT id FROM "USER" WHERE email='wXlTq@hexschooltest.io')
GROUP BY "USER".id;
...
SELECT 
    "USER".id AS user_id,
    "USER".name,
    COUNT("COURSE_BOOKING".id) AS total
FROM 
    "USER"
...
SELECT 
    purchased.user_id,
    purchased.name,
    purchased.total - (SELECT total FROM used) AS remaining_credit
FROM
    purchased;
...
SELECT
    (SELECT "USER".name FROM "USER" WHERE "USER".id = "COACH".user_id) AS 教練名稱,
...
WHERE
    purchase_at BETWEEN '2024-11-01 00:00:00' AND '2024-11-30 23:59:59'
GROUP BY 
    "CREDIT_PACKAGE".id;
...
FROM
    "CREDIT_PURCHASE"
WHERE
    purchase_at BETWEEN '2024-12-01 00:00:00' AND '2024-12-30 23:59:59';
